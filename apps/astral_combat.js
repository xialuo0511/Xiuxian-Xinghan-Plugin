  ...
  async showTeamStatus(e) {
    ...
    const helpCommands = [
      ...
      },
      {
        category: '试炼玩法',
        commands: [
          { cmd: '#开启试炼', desc: '开启新的一轮挑战' },
          { cmd: '#挑战', desc: '进入下一层战斗' },
          { cmd: '#试炼状态', desc: '查看当前状态' },
          { cmd: '#选择路线 [序号]', desc: '选择分支路线' },
          { cmd: '#选择赐福 [序号]', desc: '选择增益效果' },
          { cmd: '#刷新赐福', desc: '刷新备选赐福' },
          { cmd: '#事件选择 [序号]', desc: '事件/商店交互' },
          { cmd: '#退出试炼', desc: '结算并退出' }
        ]
      }
    ];
