...
export class Tiandibang extends plugin {
    ...
    async cansai(e) {
        ...
        db.query(sql1, async (err, result) => {
            ...
            let sql2 = `insert into tiandibang values (${usr_qq},'${player.名号}',${level_id},${player.攻击},${player.防御},${player.血量上限},${player.暴击率},${JSON.stringify(player.灵根)},${player.灵根.法球倍率},${JSON.stringify(player.学习的功法)},0,0)`
    ...
    async bd_jiesuan(e) {
        ...
        try{
            ...
            let msg=[];
            for (m = 0; m < 10; m++) {
                wuqi_name=`水脚脚的钱包x${(20-2*m)}`;
                await Add_najie_thing(tiandibang[m].qq, thing_name, thing_class, (30-3*m));
                msg.push(
                    "名次："+(m+1)+
                    "\n名号："+tiandibang[m].名号+
                    "\n积分："+tiandibang[m].积分+
