...
export class SecretPlaceTask extends plugin {
  ...
  async Secretplacetask() {
    let sql1 = `select * from action where action_mijing=1;`
    db.query(sql1, async (err, result) => {
      ...
      for (var i = 0; i < action_list.length; i++) {
        ...
        if (now_time > end_time) {
          ...
          if (action.action == "探索仙府") {
            weizhi = await data.timeplace_list.find(item => item.name == action.Place_address);
          }
          if (action.action == "镇守仙境") {
