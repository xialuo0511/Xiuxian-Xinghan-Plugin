...
效果：${upgrade.desc}`);
  ...
  async startRun(e) {
    ...
    try {
      tempClient = await getTempRedis();
      const exist = await tempClient.get(KEY_PREFIX + userId);
      if (exist) { await tempClient.disconnect(); return e.reply('你已有正在进行的试炼。'); }
      ...
      if (userData.last_fail_time && (now - userData.last_fail_time < 3 * 60 * 1000)) {
        ...
        await tempClient.disconnect(); return e.reply(`由于挑战失败，请在 ${remaining} 秒后重新开始。`);
      }
      const inputStr = e.msg.replace('#开启试炼', '').trim();
            const selectedOaths = inputStr ? inputStr.split(/[\s,，]+/).filter(Boolean) : [];      const activeOaths = []; let totalProfit = 0;
      if (selectedOaths.length > 0) {
        if (!userData.cleared) { await tempClient.disconnect(); return e.reply('只有通关一次 20 层方可开启誓约挑战。'); }
        for (const oName of selectedOaths) {
          const oath = OATHS.find(o => o.name === oName);
          if (!oath) { await tempClient.disconnect(); return e.reply(`未知誓约：【${oName}】。`); }
          if (!activeOaths.find(a => a.name === oName)) { activeOaths.push(oath); totalProfit += oath.profit; }
        }
        if (totalProfit <= 0 && activeOaths.length > 0) { await tempClient.disconnect(); return e.reply('不可只选收益为 0% 的誓约。'); }
      }
...
已激活誓约：${activeOaths.map(o => `【${o.name}】`).join('、')} (+${(totalProfit * 100).toFixed(0)}%)` : '';
      e.reply([`【万象天机·无尽试炼】已开启！`, `出战：${soulsState.map(s => s.name).join('、')}`, oathText, `第 1 层为【激战】节点，发送 #挑战 即可开始。`].join('\n'));
    } catch (err) { if (tempClient) await tempClient.disconnect(); e.reply('系统错误：' + err.message); }
  }

  generateRoutes(layer, activeOaths = []) {
    if (layer % 5 === 0) return [{ type: 'BOSS', name: '首领降临', desc: '极为危险的强敌。', rarity: 5 }];
    const options = [];
    let availableTypes = [
      { type: 'COMBAT', name: '激战', desc: '普通的战斗。', weight: 60 },
      { type: 'ELITE', name: '精英', desc: '强敌出没！', weight: 15 },
      { type: 'EVENT', name: '奇遇', desc: '未知的风险。', weight: 15 },
      { type: 'MONSTER_TREASURE', name: '盗宝妖兽', desc: '带宝流窜。', weight: 10 }
    ];
    ...
    if (layer % 5 === 4) {
      options.push({ type: 'REST', name: '修整', desc: '恢复状态。', weight: 0 });
  ...
  async processRouteGeneration(e, runData, tempClient, prefixMsg = '') {
    ...
    if (nextRoutes.length === 1) {
      ...
      e.reply(`${prefixMsg}\n\n即将进入第 ${runData.layer} 层。\n锁定路线：【${runData.current_node.name}】\n发送 #挑战 开始。`);
    } else {
      await tempClient.set(KEY_PREFIX + e.user_id, JSON.stringify(runData));
      let msg = `${prefixMsg}\n\n即将进入第 ${runData.layer} 层。请选择路线：\n`;
    ...
    try {
      ...
      if (node.type === 'SHOP') {
        if (runData.active_oaths?.some(o => o.name === '贫苦')) {
          ...
          await this.processRouteGeneration(e, runData, tempClient, '誓约·贫苦生效，商店已关闭。');
        ...
        if (!runData.artifacts.includes('treasure_bowl')) runData.shop_items.push({ type: 'artifact', id: 'treasure_bowl', name: '聚宝盆', desc: '结算收益+30%', price: 100, rarity: 3, bought: false });
      ...
      await tempClient.set(KEY_PREFIX + userId, JSON.stringify(runData));
      if (['COMBAT', 'ELITE', 'BOSS', 'MONSTER_TREASURE'].includes(node.type)) e.reply(`你选择了【${node.name}】。发送 #挑战 开始。`);
      else if (node.type === 'SHOP') {
        let msg = `持有印：${runData.jing_yin}\n`;
      ...
      } else if (node.type === 'REST') e.reply('营地：\n1. 修养(40%HP)\n2. 招魂(复活)\n3. 冥想(+1刷新)\n#事件选择 [序号]');
      else if (node.type === 'EVENT') {
        const sub = runData.current_node.sub_type;
        if (sub === 'vending_machine_gold') e.reply('抽奖机：\n1. 抽奖(100印，限3次)\n2. 离开');
        else if (sub === 'vending_machine_weird') e.reply('奇怪机：\n1. 抽奖(25印，限3次)\n2. 离开');
        else if (sub === 'gamble_all') e.reply('契约：\n1. 放手一搏(99%HP换属性)\n2. 离开');
        else if (sub === 'ultimate_boost') e.reply('圣光：\n1. 顶级强化(全复活全满强化)\n2. 离开');
        else if (sub === 'soul_enhance') e.reply('老者：\n1. 虚心求教(专属强化)\n2. 离开');
        else e.reply('售货机：\n1. 补给(20%HP换3赐福)\n2. 暴力破解\n3. 离开');
      }
      await tempClient.disconnect();
  ...
  async handleEventChoice(e) {
    ...
    try {
      ...
      if (node.type === 'SHOP') {
        ...
        else if (selection === items.length + 1) { e.reply('刷新商品中...'); return this.selectRoute(e); }
        else if (selection >= 1 && selection <= items.length) {
          const it = items[selection - 1];
          if (it.bought) e.reply('售罄');
          else if (runData.jing_yin < it.price) e.reply('印不足');
        ...
        else if (selection === 3) { runData.refresh_count++; replyMsg = '刷新次数+1。'; isDone = true; }
      } else if (node.type === 'EVENT') {
        ...
        if (sub === 'vending_machine_gold' && selection === 1) {
          ...
          else if (r < 0.7) { const b = BUFFS.filter(b => b.rarity <= 3)[0]; runData.buffs.push(b.id); replyMsg = `得【${b.name}】`; }
          else replyMsg = '空的。';
...
剩 ${3 - node.event_count} 次`); return; }
          ...
          else if (r < 0.4) { runData.extra_atk_pct = (runData.extra_atk_pct || 0) + 1.0; replyMsg = '全攻击+100%'; }
          else if (r < 0.6) { const oath = OATHS[Math.floor(Math.random() * OATHS.length)]; runData.active_oaths.push({ ...oath, profit: oath.profit + 0.1 }); replyMsg = `得誓约【${oath.name}】`; }
          else replyMsg = '无事。';
        ...
        else if (sub === 'ultimate_boost' && selection === 1) { runData.souls.forEach(s => { s.is_dead = false; s.current_hp = s.max_hp; const b = BUFFS.find(b => b.type === 'soul_exclusive' && b.exclusive_soul === s.name && b.rarity === 3); if (b && !runData.buffs.includes(b.id)) runData.buffs.push(b.id); }); replyMsg = '全满。'; isDone = true; }
  ...
  async challengeLayer(e) {
    ...
    try {
      ...
      if (!['COMBAT', 'ELITE', 'BOSS', 'MONSTER_TREASURE'].includes(node.type)) { await tempClient.disconnect(); return e.reply('不可战。'); }
      ...
      e.reply(`【${node.name}】战斗开始...`);
      const result = await runCombat(battleSouls, enemies, runData.buffs, 20);
      const slices = []; let currentSlice = [], rCount = 0;
      for (const entry of result.log) { if (entry.type === 'turn') { rCount++; if (rCount > 8) { slices.push(currentSlice); currentSlice = []; rCount = 1; } } currentSlice.push(entry); }
      if (currentSlice.length) slices.push(currentSlice);
      ...
      for (let i = 0; i < slices.length; i++) {
        ...
        if (buf) {
          ...
          try { const sR = await e.reply(segment.image(fP)); if (!sR || sR.result === -1) await e.reply({ type: 'file', file: fP, name: path.basename(fP) }); } catch (err) { await e.reply({ type: 'file', file: fP, name: path.basename(fP) }); }
          setTimeout(() => { if (fs.existsSync(fP)) fs.unlinkSync(fP); }, 60000);
        }
        if (i < slices.length - 1) await new Promise(r => setTimeout(r, 1000));
      }
      ...
      if (result.playerWon) {
        ...
        let extra = '';
        if (node.type === 'MONSTER_TREASURE') { const k = result.enemyTeam.filter(et => et.current_hp <= 0 && !et.has_fled).length; gold = k === 3 ? 300 : 250; extra = `\n击败 ${k} 妖兽，获 ${gold} 印！`; }
        else { const kE = result.enemyTeam.filter(et => et.name === '盗宝妖兽' && et.current_hp <= 0 && !et.has_fled).length; if (kE > 0) { gold += 50; extra = `\n额获 50 印！`; } }
        runData.jing_yin += gold; runData.temp_jade += jade;
        if (runData.layer >= 20) {
          const ud = await this.getUserData(tempClient, userId); const b = runData.total_profit >= 1.0 ? 150 : 0;
          ud.jade += runData.temp_jade + b; ud.cleared = true;
          await this.saveUserData(tempClient, userId, ud); await tempClient.del(KEY_PREFIX + userId);
          e.reply(`【通关】获 ${runData.temp_jade} 玉。`);
        } else {
          ...
          const pool = BUFFS.filter(b => b.type !== 'artifact_passive' && !runData.buffs.includes(b.id) && w[b.rarity] > 0);
          const ch = [];
          ...
          await tempClient.set(KEY_PREFIX + userId, JSON.stringify(runData));
          let bMsg = `【${node.name}】捷！${extra}\n获印：${gold}。当前：${runData.jing_yin}。\n\n【赐福】可选 ${runData.remaining_picks}：\n`;
        ...
        await this.saveUserData(tempClient, userId, ud); await tempClient.del(KEY_PREFIX + userId);
        e.reply(`败！获 ${runData.temp_jade} 玉。冷静 3 分钟。`);
      }
      await tempClient.disconnect();
  ...
  async selectBuff(e) {
    ...
    try {
      ...
      if (runData.remaining_picks <= 0) { runData.pending_buffs = []; await this.processRouteGeneration(e, runData, tempClient, `选定【${b.name}】！`); } 
      else { await tempClient.set(KEY_PREFIX + userId, JSON.stringify(runData)); e.reply(`选定【${b.name}】，还可选 ${runData.remaining_picks} 个。`); }
      await tempClient.disconnect();
    } catch (err) { if (tempClient) await tempClient.disconnect(); }
  }
    ...
    try {
      ...
      if (dataStr) {
        ...
        if (runData.temp_jade > 0) {
            ...
            e.reply(`退出成功。获得 ${runData.temp_jade} 天机玉。`);
        } else e.reply('已退出。');
