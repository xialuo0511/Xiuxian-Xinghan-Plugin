...
效果：${upgrade.desc}`);
  ...
  async startRun(e) {
    ...
    try {
      ...
      if (soulsState.length === 0) { await tempClient.disconnect(); return e.reply('你没有装备任何星魂，或者由于誓约限制无星魂可出战。'); }
...
'));
  ...
  async processRouteGeneration(e, runData, tempClient, prefixMsg = '') {
    ...
    if (nextRoutes.length === 1) {
      ...
      e.reply(`${prefixMsg}\n\n即将进入第 ${runData.layer} 层。\n已自动锁定路线：【${runData.current_node.name}】\n发送 #挑战 开始挑战！`);
    } else {
      await tempClient.set(KEY_PREFIX + e.user_id, JSON.stringify(runData));
      let msg = `${prefixMsg}\n\n即将进入第 ${runData.layer} 层。\n请选择前行方向：\n`;
  ...
  async selectRoute(e) {
    ...
    try {
      ...
      if (['COMBAT', 'ELITE', 'BOSS', 'MONSTER_TREASURE'].includes(node.type)) e.reply(`你选择了【${node.name}】。\n敌人已在前方，发送 #挑战 开始战斗！`);
        ...
        runData.shop_items.forEach((it, i) => msg += `${i + 1}. 【${it.name}】${it.bought ? '(已售罄)' : `💰${it.price}`}\n   ${it.desc}\n`);
        e.reply(msg + `\n${runData.shop_items.length + 1}. 【刷新】 更换一批商品
${runData.shop_items.length + 2}. 【离开】 继续前进
发送 #事件选择 [序号] 购买或离开。`);
      } else if (node.type === 'REST') e.reply('你来到了一处营地。
1. 【休养生息】 全队恢复 40% 生命值
...
2. 【无视】');
        else if (sub === 'ultimate_boost') e.reply('一道圣光从天而降！
1. 【顶级强化】 复活全员，恢复满状态，并获得全员专属三星赐福！
2. 【离开】');
        else if (sub === 'soul_enhance') e.reply('你在废墟中遇到一位神秘的老者。
  ...
  async handleEventChoice(e) {
    ...
    try {
      ...
      if (node.type === 'SHOP') {
        ...
        else if (selection === items.length + 1) { e.reply('正在刷新商品...'); return this.selectRoute(e); }
        ...
        else if (selection === 2) { const dead = runData.souls.filter(s => s.is_dead); if (dead.length) { const s = dead[Math.floor(Math.random() * dead.length)]; s.is_dead = false; s.current_hp = Math.floor(s.max_hp * 0.5); replyMsg = `【${s.name}】已复活。`; } else replyMsg = '无人阵亡，你只是休息了一会儿。'; isDone = true; }
...
还可以抽奖 ${3 - node.event_count} 次。`); return; }
          ...
          if (r < 0.1) { runData.souls.forEach(s => s.current_hp = Math.max(1, Math.floor(s.current_hp * 0.9))); replyMsg = '诡异的烟雾让你感到虚弱 (生命值-10%)。'; }
...
还可以抽奖 ${3 - node.event_count} 次。`); return; }
  ...
  async challengeLayer(e) {
    ...
    try {
      tempClient = await getTempRedis();
      const runData = JSON.parse(await tempClient.get(KEY_PREFIX + userId));
      if (!runData?.current_node) { await tempClient.disconnect(); return e.reply('请先选择路线。'); }
      const node = runData.current_node;
      if (!['COMBAT', 'ELITE', 'BOSS', 'MONSTER_TREASURE'].includes(node.type)) { await tempClient.disconnect(); return e.reply('当前节点不可进行挑战。'); }
      ...
      const slices = []; let currentSlice = [], roundCountInSlice = 0;
      for (const entry of result.log) {
        if (entry.type === 'turn') { roundCountInSlice++; if (roundCountInSlice > 8) { slices.push(currentSlice); currentSlice = []; roundCountInSlice = 1; } }
      ...
      for (let i = 0; i < slices.length; i++) {
        const renderData = { log: slices[i], pluResPath: `file://${process.cwd()}/plugins/xiuxian-emulator-plugin/resources/` };
        const dataForPuppeteer = await new Show(e).get_imgData('astral_combat_log', renderData);
        dataForPuppeteer.imgType = 'jpeg'; dataForPuppeteer.quality = 80;
        const imgResult = await puppeteer.screenshot('astral_combat_log', { ...dataForPuppeteer });
        let finalBuffer = Buffer.isBuffer(imgResult) ? imgResult : (imgResult?.file ? Buffer.from(imgResult.file.replace(/^base64:\\/, '').replace(/^data:image\/\w+;base64,/, ''), 'base64') : null);
        if (finalBuffer) {
          const filePath = path.join(tempDir, `Log_${userId}_${Date.now()}_P${i + 1}.jpg`);
          fs.writeFileSync(filePath, finalBuffer);
          try { const sendRes = await e.reply(segment.image(filePath)); if (!sendRes || sendRes.result === -1) await e.reply({ type: 'file', file: filePath, name: path.basename(filePath) }); } 
          catch (err) { await e.reply({ type: 'file', file: filePath, name: path.basename(filePath) }); }
      ...
      if (result.playerWon) {
        ...
        let extraMsg = '';
        if (node.type === 'MONSTER_TREASURE') { const killed = result.enemyTeam.filter(et => et.current_hp <= 0 && !et.has_fled).length; gold = killed === 3 ? 300 : 250; extraMsg = `\n击败了 ${killed} 只妖兽，获得 ${gold} 天机印！`; }
        else { const killedExtra = result.enemyTeam.filter(et => et.name === '盗宝妖兽' && et.current_hp <= 0 && !et.has_fled).length; if (killedExtra > 0) { gold += 50; extraMsg = `\n额外击败了乱入的妖兽，获得 50 天机印！`; } }
        ...
        if (runData.layer >= 20) {
          ...
          let weights = isBossNode ? { 1: 0, 2: 0, 3: 75, 4: 25 } : { 1: 80, 2: 40, 3: 10, 4: 0 };
          const pool = BUFFS.filter(b => b.type !== 'artifact_passive' && !runData.buffs.includes(b.id) && weights[b.rarity] > 0);
          const choices = [];
          for (let i = 0; i < 3 && pool.length > 0; i++) {
              let total = pool.reduce((acc, b) => acc + weights[b.rarity], 0); let r = Math.random() * total;
    ...
    try {
      ...
      if (dataStr) {
        ...
        if (runData.temp_jade > 0) {
            ...
            e.reply(`已退出。获得 ${runData.temp_jade} 天机玉。`);
        } else e.reply('已退出。');
