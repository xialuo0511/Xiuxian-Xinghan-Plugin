...
export class WanxiangActivity extends plugin {
  ...
  async viewOaths(e) {
    ...
    try {
      ...
      const img = await puppeteer.screenshot('wanxiang_oaths', { tplFile: htmlPath, ...renderData, imgType: 'png' });
      await e.reply(img);
    } catch (err) {
  ...
  async showExchangeShop(e) {
    ...
    try {
      ...
      const img = await puppeteer.screenshot('wanxiang_shop', { tplFile: htmlPath, ...renderData, imgType: 'png' });
      await e.reply(img);
      await tempClient.disconnect();
    } catch (err) {
      if (tempClient) await tempClient.disconnect();
  ...
  async viewSecrets(e) {
    ...
    try {
      ...
      const img = await puppeteer.screenshot('wanxiang_secrets', { tplFile: htmlPath, ...renderData, imgType: 'png' });
      await e.reply(img);
...
已激活誓约：${activeOaths.map(o => `【${o.name}】`).join('、')} (结算收益 +${(totalProfit * 100).toFixed(0)}%)` : '';
  ...
  async showStatus(e) {
    ...
    try {
      ...
      const img = await puppeteer.screenshot('wanxiang_status', { ...dFP, imgType: 'png' }); await e.reply(img);
    } catch (err) { if (tempClient) await tempClient.disconnect(); }
  }
