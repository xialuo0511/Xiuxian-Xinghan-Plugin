...
export class Garden extends plugin {
    ...
    async Vegetable(e) {
        ...
        if (!isNotNull(player.宗门)) { 
            return;
        }else if ((ass.药园.药园等级)==1 ||((ass.药园.药园等级) !== ass.宗门等级)) {//加入宗门，没有药园或药园等级不等于宗门等级，则新建药园。
    ...
    async Get_vegetable(e) {
        ...
        if (!isNotNull(player.宗门)) {
            return;
        }else if ((ass.药园.药园等级)==1) {//加入宗门，没有药园，则新建药园。
        ...
        let transferTimeout = parseInt( 60000 * time )//
          if (nowTime < last_garden_time + transferTimeout) {
            ...
            e.reply(`每${transferTimeout / 1000 / 60}分钟拔苗一次。` + `cd: ${waittime_m}分${waittime_s}秒`);
            return;
        }
        ...
        for (var i = 0; i < vegetable.length; i++) {
            if (vegetable[i].name == vagetable_name) {
                ...
                if(nowTime + 1000*60*30 < vegetable_Oldtime ){ //判断是否成熟
                    e.reply(`作物${vagetable_name}增加1800000成熟度,还需要${vegetable_Oldtime-nowTime-1000*60*30}成熟度`);
    ...
    async Silencing (e) {
        ...
        for (let msg of e.message) {//获取对方QQ
            if (msg.type == 'at') {
                qq = msg.qq;
                break;
            }
        ...
        if (qingdianshu !== false && qingdianshu !==0 ){//判断纳戒有没有剑帝信物
            ...
            if (num1 < num2) {
                if(num3 > num4){
                    ...
                    e.reply(`${user_id}${player.名号}一手按压在剑帝信物上，一道剑光从剑帝信物上发出，化成符文冲入${qq}${player1.名号}口中，封！`)
                ...
                if(num3 > num4){
                    ...
                    e.reply(`一手按压在剑帝信物上，一道剑光从剑帝信物上发出，化成符文冲入${qq}${player1.名号}口中，封！`)
                    e.group.muteMember(qq, num2 * 8);
                }
