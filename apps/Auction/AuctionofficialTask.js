...
export class AuctionofficialTask extends plugin {
  ...
  //     let auctionENDTIME = await redis.get('xiuxian:AuctionofficialTaskENDTIME');
  //     let nowTime = new Date().getTime();
  //     if (nowTime >= auctionENDTIME) {
  //       await openAU();
  //       await redis.set(
  //         'xiuxian:AuctionofficialTaskENDTIME',
  //         JSON.stringify(auctionENDTIME + 7 * 24 * 60 * 60 * 1000)
  //       );
  //     }
  ...
  //       );
  //       msg = `星阁限定物品【${auction.thing.name}】拍卖中\n距离拍卖结束还有${m}分${s}秒\n目前最高价${auction.last_price}`;
  ...
  //         msg = `流拍，${auction.thing.name}已退回神秘人的纳戒`;
  //       } else {
