...
export async function handleQiBreakthrough(userId, useLuck = false) {
  ...
  const lastTime = parseInt(await redis.get(`xiuxian:player:${userId}:last_Levelup_time`)) || 0;
  ...
  const transactionSuccess = await DAL.transaction_update(userId, (player) => {
    ...
    if (useLuck) {
      prob += (1 - prob) * 0.5; // 幸运草减少50%的失败率
    }
    if (player.breakthrough) {
      prob += 0.2;
      player.breakthrough = false; // 消耗破境丹效果
    }
    ...
    if (rand > prob) {
      ...
      return 'failed_but_updated'; // 特殊返回值，表示失败但数据已修改
    }
...
export async function handleBodyBreakthrough(userId, useLuck = false) {
  const nowTime = Date.now();
  const cdTime = xiuxianConfigData.CD.level_up * 60000;
  const lastTime = parseInt(await redis.get(`xiuxian:player:${userId}:last_LevelMaxup_time`)) || 0;
  ...
  const transactionSuccess = await DAL.transaction_update(userId, (player) => {
    ...
    if (rand > prob) {
      ...
      if (lostExp > 0) player.血气 -= lostExp;
